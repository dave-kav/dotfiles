            ...
            if id == "__new__" then
                child_window:perform_action(
                    wezterm.action.PromptInputLine({
                        ...
                        action = wezterm.action_callback(function(w, p, input)
                            if not input or input == "" then
                                return
                            end
                            -- Expand ~ manually
                            local expanded = input:gsub("^~/", wezterm.home_dir .. "/")
                            local is_path = expanded:sub(1, 1) == "/"
                            local name = is_path and expanded:match("([^/]+)$") or input
                            ...
                            w:perform_action(
                                wezterm.action.SwitchToWorkspace({ name = name, spawn = spawn }),
                                p
                            )
